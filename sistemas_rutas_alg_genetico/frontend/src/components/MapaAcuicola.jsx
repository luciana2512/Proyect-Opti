  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 text-xs items-center">
        ...
        {geometriaEscenario?.length > 2 && (
          <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border bg-card border-border/70">
            <span style={{ display: 'inline-block', width: '10px',
                           borderTop: '2px dashed #f59e0b' }} />
            <span className="font-medium text-[11px]" style={{ color: '#f59e0b' }}>
              Ruta escenario
            </span>
          </span>
        )}
      ...
      <div className="rounded-xl overflow-hidden border border-border/50" style={{ height: 520 }}>
        <MapContainer center={[5.0265, -74.747]} zoom={6} style={{ height: '100%', width: '100%' }}>
          ...
          {/* ── Ruta bajo escenario What-If (ámbar discontinuo) ─── */}
          {geometriaEscenario?.length > 2 && (
            <Polyline
              positions={geometriaEscenario}
              pathOptions={{
                color: '#f59e0b',
                weight: 5,
                opacity: 0.9,
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-xs font-medium text-muted-foreground px-1">
        ...
        {geometriaEscenario?.length > 2 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-amber-600 font-semibold text-xs">Ruta bajo escenario</span>
          </div>
        )}
      </div>
    </div>
