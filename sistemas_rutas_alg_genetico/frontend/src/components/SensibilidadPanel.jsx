...
function ResultadoComparativo({ resultado }) {
  ...
  return (
    <div data-tour="comparativa-resultados" className="mt-4 space-y-4">
          ...
          <div className={`rounded-xl border-2 p-4 ${
            ...
            <div className="flex items-end justify-between flex-wrap gap-3">
              ...
              <div className="text-center flex-1 min-w-[100px]">
                ...
                {deltaPct != null && (
                  <p className={`text-xs font-bold mt-0.5 ${delta < 0 ? 'text-red-500' : 'text-green-600 dark:text-green-400'}`}>
      ...
      <div className={`rounded-xl border p-4 ${
          !nuevo.factible          ? 'border-red-500/30 bg-red-500/5'
          : !nuevo.viable          ? 'border-orange-500/30 bg-orange-500/5'
          :                          'border-border bg-card/60'
      }`}>
        <div className="flex items-center justify-between mb-2">
          <span className="font-semibold text-sm">Estado del sistema</span>
          <Badge variant="outline" className={
              !nuevo.factible ? 'border-red-500/30 text-red-600 bg-red-500/10'
              : !nuevo.viable ? 'border-orange-500/30 text-orange-600 bg-orange-500/10'
              :                 'border-green-500/30 text-green-600 bg-green-500/10'
          }>
            {!nuevo.factible ? 'Infactible' : !nuevo.viable ? 'Inviable económicamente' : 'Factible'}
          </Badge>
        </div>
        <p className="text-xs text-muted-foreground">{impacto.mensaje}</p>
        {nuevo.factible && !nuevo.viable && nuevo.razon_inviable && (
          <p className="text-xs text-orange-600 font-medium mt-1">
            ⚠️ {nuevo.razon_inviable}
          </p>
        )}
      </div>
...
/* ─── Panel principal ───────────────────────────────────────────────────────── */
const NOMBRES_ESCENARIO = { 1: 'Alza de Combustible', 2: 'Cierre de Vía', 3: 'Pérdida de Calidad' };

export default function SensibilidadPanel({
  origenRuta = 'O1',
  destinoRuta = 'D1',
  onGeometriaEscenario,
  escenarioActivo,
  onVerEnMapa,
}) {
  ...
  const [geometriasEscenario, setGeometriasEscenario] = useState({});
  ...
  useEffect(() => {
    if (onGeometriaEscenario) {
      const ultimoEscenario = Object.keys(geometriasEscenario).pop();
      if (ultimoEscenario) {
        onGeometriaEscenario(
          Number(ultimoEscenario),
          geometriasEscenario[ultimoEscenario]
        );
      }
    }
  }, [geometriasEscenario]);
  ...
  const ejecutarEscenario = async (numero) => {
    ...
    try {
      const res = await axios.get(`/api/acuicola/sensibilidad/${numero}`, {
        params: { ...params[numero], _t: Date.now() },
      });
      setResultados(prev => ({ ...prev, [numero]: res.data }));
      // Calcular impacto en la ruta seleccionada
      const datosEscenario = res.data;
      const paramRuta = { origen: origenRuta, destino: destinoRuta, escenario: numero, ...params[numero], _t: Date.now() };
      axios.get('/api/acuicola/ruta_escenario', { params: paramRuta })
        .then(r => {
          setRutasImpacto(prev => ({ ...prev, [numero]: r.data }));
          if (datosEscenario.resultado_nuevo?.factible) {
            onVerEnMapa?.({
              flujos:          datosEscenario.resultado_nuevo.flujos || {},
              cuellos:         datosEscenario.resultado_nuevo.cuellos_botella || [],
              rutaEscenario:   r.data?.ruta_escenario?.factible ? (r.data.ruta_escenario.ruta || null) : null,
              nombreEscenario: `Escenario #${numero} — ${NOMBRES_ESCENARIO[numero]}`,
            });
          }
          const rutaMod = r.data?.ruta_escenario?.ruta;
          if (rutaMod?.length >= 2) {
            axios.get('/api/acuicola/geometria_ruta', {
              params: { nodos: rutaMod.join(',') },
              timeout: 20000,
            }).then(gr => {
              const pts = gr.data?.geometria || [];
              if (pts.length > 2) {
                setGeometriasEscenario(prev => ({ ...prev, [numero]: pts }));
              }
            }).catch(() => {});
          }
        })
  ...
  const ejecutarTodos = async () => {
    ...
    try {
      ...
      const [r1, r2, r3] = await Promise.all([
        axios.get('/api/acuicola/sensibilidad/1', { params: { ...params[1], _t: t } }),
        axios.get('/api/acuicola/sensibilidad/2', { params: { ...params[2], _t: t } }),
        axios.get('/api/acuicola/sensibilidad/3', { params: { ...params[3], _t: t } }),
      ]);
      ...
      Promise.all([1,2,3].map(n =>
        axios.get('/api/acuicola/ruta_escenario', {
          params: { origen: origenRuta, destino: destinoRuta, escenario: n, ...params[n], _t: t }
          ...
          if (r) {
            ...
            if (rutaMod?.length >= 2) {
              axios.get('/api/acuicola/geometria_ruta', {
                ...
                const pts = gr.data?.geometria || [];
                if (pts.length > 2) {
                  setGeometriasEscenario(prev => ({ ...prev, [r.n]: pts }));
                }
              }).catch(() => {});
            }
          }
  ...
  return (
    <div className="space-y-6">
      ...
      <div data-tour="escenarios-sensibilidad" className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {ESCENARIOS.map(esc => {
          ...
          return (
            <Card key={esc.numero} className={`border ${colors.border} transition-all`}>
              ...
              <CardContent className="pt-0 space-y-3">
                ...
                {/* ── Ver en mapa ───────────────────────────────────── */}
                {resultado && !resultado.error && resultado.resultado_nuevo?.factible && (
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full gap-2 border-blue-500/40 text-blue-600 dark:text-blue-400 hover:bg-blue-500/10"
                    onClick={() => onVerEnMapa?.({
                      flujos:          resultado.resultado_nuevo.flujos || {},
                      cuellos:         resultado.resultado_nuevo.cuellos_botella || [],
                      rutaEscenario:   rutasImpacto[esc.numero]?.ruta_escenario?.factible
                                         ? (rutasImpacto[esc.numero].ruta_escenario.ruta || null)
                                         : null,
                      nombreEscenario: `Escenario #${esc.numero} — ${esc.titulo}`,
                    })}
                  >
                    🗺 Ver en mapa
                  </Button>
                        ...
                        <td className="py-2.5 px-3 text-center">
                          <Badge variant="outline" className={
                            !r.resultado_nuevo?.factible
                              ...
                              : !r.resultado_nuevo?.viable
                              ? 'border-orange-500/30 text-orange-600 bg-orange-500/10 text-xs'
