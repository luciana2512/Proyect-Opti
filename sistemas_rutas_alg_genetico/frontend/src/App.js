...
import TourGuide from './components/TourGuide'; // ← NUEVA IMPORTACIÓN
...
function App() {
  ...
  const [geometriaEscenario, setGeometriaEscenario] = useState([]);
  const [escenarioActivoNum, setEscenarioActivoNum] = useState(null);
  const [escenarioMapa,      setEscenarioMapa]      = useState(null);
  ...
  return (
    <Router>
      <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
        ...
        {/* ── Header ─────────────────────────────────────────────────── */}
        <header className="sticky top-0 z-50 glass-effect border-b" data-tour="header">
            ...
            <div className="flex gap-1 mt-4">
              {TABS.map(tab => {
                ...
                return (
                  <button
                    key={tab.id}
                    data-tour={tab.id === 'red' ? 'tab-red' : 'tab-sensibilidad'}
        ...
        <main className="max-w-[1400px] mx-auto px-6 py-6">
          <div style={{ display: tabActiva === 'red' ? 'block' : 'none' }}>
            <AcuicolaPanel
              ...
              escenarioMapa={escenarioMapa}
              onLimpiarEscenario={() => setEscenarioMapa(null)}
            />
          </div>
          <div style={{ display: tabActiva === 'sensibilidad' ? 'block' : 'none' }}>
            <SensibilidadPanel
              origenRuta={origenRuta}
              destinoRuta={destinoRuta}
              onGeometriaEscenario={(num, pts) => {
                setEscenarioActivoNum(num);
                setGeometriaEscenario(pts);
              }}
              escenarioActivo={escenarioActivoNum}
              onVerEnMapa={(data) => { setEscenarioMapa(data); setTabActiva('red'); }}
            />
          </div>
        </main>
