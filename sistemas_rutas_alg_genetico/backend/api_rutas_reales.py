...
from grafo_acuicola import (
    ...
    obtener_geometria_ruta_real, obtener_geometria_arista,
...
@app.route('/api/acuicola/cargar_red', methods=['POST'])
def cargar_red_usuario():
    """
    Recibe un archivo JSON con una red logística personalizada y la carga en
    memoria, reemplazando la red activa para toda la sesión.

    El JSON debe tener la misma estructura que red_acuicola.json:
        { "nodos": [...], "aristas": [...], "nombre": "..." }

    Body: multipart/form-data con campo "archivo" (archivo .json).
    """
    try:
        if 'archivo' not in request.files:
            return jsonify({'error': 'Se esperaba el campo "archivo" en el formulario.'}), 400
        ...
        try:
            red_nueva = json.loads(contenido.decode('utf-8'))
        except json.JSONDecodeError as je:
            return jsonify({'error': f'JSON inválido: {je}'}), 400

        if 'nodos' not in red_nueva or 'aristas' not in red_nueva:
            return jsonify({'error': 'El JSON debe contener los campos "nodos" y "aristas".'}), 400

        _cache['red'] = red_nueva

        n_origenes  = sum(1 for n in red_nueva['nodos'] if n.get('tipo') == 'origen')
        n_transitos = sum(1 for n in red_nueva['nodos'] if n.get('tipo') == 'transito')
        n_destinos  = sum(1 for n in red_nueva['nodos'] if n.get('tipo') == 'destino')

        print(f'[cargar_red] Nueva red: {len(red_nueva["nodos"])} nodos, {len(red_nueva["aristas"])} aristas')

        return jsonify({
            'ok':        True,
            'mensaje':   f'Red "{red_nueva.get("nombre", "sin nombre")}" cargada en memoria.',
            'nombre':    red_nueva.get('nombre', 'Red personalizada'),
            'n_nodos':   len(red_nueva['nodos']),
            'n_aristas': len(red_nueva['aristas']),
            'resumen': {
                'n_origenes':    n_origenes,
                'n_transitos':   n_transitos,
                'n_destinos':    n_destinos,
                'oferta_total':  sum(n.get('oferta', 0)  for n in red_nueva['nodos'] if n.get('tipo') == 'origen'),
                'demanda_total': sum(n.get('demanda', 0) for n in red_nueva['nodos'] if n.get('tipo') == 'destino'),
            },
        })
...
def distribucion_local(transito_id):
    ...
    try:
        ...
        resultado = optimizar_distribucion_local(
            ...
            modo=request.args.get('modo', 'local'),
        )
        return jsonify(resultado)
    except Exception as e:
        return jsonify({'error': str(e), 'traceback': traceback.format_exc()}), 500
...
def ruta_escenario():
    ...
    try:
        ...
        if escenario == 1:
            ...
            nodo_id = request.args.get('nodo_id', 'T1')
            factor  = float(request.args.get('factor_penalizacion', 3.0))
            calidad_degradada = 0.35
            umbral_calidad    = 0.40

            if calidad_degradada < umbral_calidad:
                # Calidad bajo umbral mínimo → bloqueo completo,
                # consistente con el comportamiento del LP cuando
                # un nodo cae por debajo del umbral de calidad.
                aristas_bloqueadas = [
                    a['id'] for a in red['aristas'] if a['origen'] == nodo_id
                ]
            else:
                # Calidad penalizada pero sobre umbral → encarece
                # el costo sin bloquear, igual que hace el LP.
                factores_costo = {
                    a['id']: factor
                    for a in red['aristas']
                    if a['origen'] == nodo_id
...
if __name__ == '__main__':
    ...
    print('  POST /api/acuicola/cargar_red              (multipart: archivo=<JSON>)')
